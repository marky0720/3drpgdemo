...
void MyWeapon::useParticleEffect()
{
...
void MyWeapon::ChangeWeapon(int id)
{
	...
	weaponNode->setMaterialFlag(video::EMF_ANTI_ALIASING,true);//开启抗锯齿
...
void MyWeapon::setUseHLSL(bool b,bool transparentMeta,int weaponType)
{
  usehlsl=b;
  if(b){
	//HLSL();
...
void MyWeapon::useBillboard()
{
		...
		if(pJointNode)
		{
			...
			printf("%3.1f %3.1f %3.1f\n%3.1f %3.1f %3.1f\n ",pos1.X,pos1.Y,pos1.Z,pos2.X,pos2.Y,pos2.Z);
				ISceneNode*Plane;
				Plane =  smgr->addPlaneSceneNode(start,end,4,pJointNode)  ;//smgr->addBillboardSceneNode(pJointNode, core::dimension2d<f32>(8, 22));
			//Plane->setPosition(vector3df(0,0,0));
			Plane->setRotation(vector3df(0,0,90));
			...
			Plane =  smgr->addPlaneSceneNode(start,end,4,pJointNode)  ;
			Plane->setRotation(vector3df(0,0,0));
			Plane->setMaterialFlag(EMF_LIGHTING, false);
			Plane->setMaterialFlag(EMF_BACK_FACE_CULLING,false); 
			//node->setRotation(vector3df(90,90,90));
			Plane->setMaterialType(video::EMT_TRANSPARENT_ADD_COLOR ); 

			glow = smgr->createTextureAnimator(textures, 100);
...
void MyWeapon::HLSL(){
	...
	switch(driverType)
	{
		case video::EDT_DIRECT3D9:
				psFileName = "hlsleffect/xbjWeapon.hlsl";
	...
	if (gpu)
	{
		...
		newMaterialType1 = gpu->addHighLevelShaderMaterialFromFiles(
			getWeaponXBJHLSLFile(device->getFileSystem()), "vertexMain", video::EVST_VS_1_1,
			getWeaponXBJHLSLFile(device->getFileSystem()), "pixelMain", video::EPST_PS_1_1,
			mc, video::EMT_TRANSPARENT_ADD_COLOR);//EMT_TRANSPARENT_ADD_COLOR EMT_SOLID
